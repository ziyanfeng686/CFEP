...
hla_sequence = pd.read_csv(r'./data/CFEPII/common_hla_sequence.csv')
...
def data_with_loader(type_ = 'train',fold = None,  batch_size = 1024):
    if type_ != 'train' and type_ != 'val':
        data = pd.read_csv('./data/CFEPII/{}_data.csv'.format(type_), index_col = False)

    elif type_ == 'train':
        data = pd.read_csv('./data/CFEPII/train_data.csv', index_col = False)
    elif type_ == 'val':
        data = pd.read_csv('./data/CFEPII/val_0.csv', index_col = False)
        
    pep_inputs, hla_inputs, labels = make_data(data)
    loader = Data.DataLoader(MyDataSet(pep_inputs, hla_inputs, labels), batch_size, shuffle = False, drop_last = True, num_workers = 0)
    
    return data, pep_inputs, hla_inputs, labels, loader



#independent_data, independent_pep_inputs, independent_hla_inputs, independent_labels, independent_loader = data_with_loader(type_ = 'independ',fold = None,  batch_size = batch_size)
...
for fold in range(0,1):
    ...
    print('-----Train-----')
    dir_saver = r'./checkpoints/CFEPII'
    path_saver = r'./checkpoints/CFEPII/fold{}.pkl'.format(fold)
